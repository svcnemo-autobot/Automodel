---
title: "Dataset Overview: LLM, VLM, and Retrieval Datasets"
description: "Choose and configure datasets for language, vision-language, retrieval, and diffusion training."
position: 1
---
...
- Refer to [LLM datasets](/datasets/text-dataset), [VLM datasets](/datasets/multi-modal-dataset), [Retrieval dataset](/datasets/retrieval-dataset), and [Retrieval fine-tuning](/recipes-e2e-examples/retrieval-finetune) for deeper, task-specific guidance.
...
dataset:
  ...
  start_of_turn_token: "<|assistant|>"
```
Refer to the [Column-Mapped Text Instruction Dataset](/datasets/columnmapped-dataset) for more information.
...
```
Refer to the [Function Calling guide](/recipes-e2e-examples/function-calling) for an end-to-end example with FunctionGemma.
For a small reasoning-style chat SFT starting point, refer to [qwen2_5_0p5b_instruct_fineproofs_chat.yaml](https://github.com/NVIDIA-NeMo/Automodel/blob/main/examples/llm_finetune/qwen/qwen2_5_0p5b_instruct_fineproofs_chat.yaml).
...
dataloader:
  ...
  shuffle: true
```
Refer to [Retrieval dataset](/datasets/retrieval-dataset) for schema details and [Retrieval fine-tuning](/recipes-e2e-examples/retrieval-finetune) for the end-to-end workflow.
...
```
Refer to the [pretraining guide](/recipes-e2e-examples/pretraining), which uses MegatronPretraining data.
...
Refer to [Gemma-3n](/recipes-e2e-examples/gemma-3-3n) and [VLM dataset](/datasets/multi-modal-dataset) for end-to-end examples.
...
Refer to the [Diffusion Dataset Preparation](/datasets/diffusion-dataset) guide for full preprocessing instructions and configuration details.
